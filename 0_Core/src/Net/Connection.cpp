...
bool CConnection::Receive(CMessageFactory& factory, std::vector<CMessage*>& messages, unsigned char* buffer, size_t size)
{
	CPacket packet;
	...
	packet.Serialize(stream, factory);
...
bool CConnection::Send(CMessageFactory& factory, std::vector<CMessage*>& messages, bool SPL /* = false */)
{
	...
	// Create a new packet
	CPacket packet(m_localSequenceNum, m_remoteSequenceNum, ackBitfieldInt, messages);
